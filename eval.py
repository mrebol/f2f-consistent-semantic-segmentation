...
def eval(val_loader, net, tensorboard, device, batch_nr_training, image_save_path, save_pred_color=False,
    ...
    dataset_name = os.path.basename(val_loader.dataset.root[0]) + '_' + val_loader.dataset.split[0]
    if save_pred_color:
        image_save_color_path = os.path.join(image_save_path, "{}-dataset_color".format(dataset_name))
        os.makedirs(image_save_color_path, exist_ok=True)
    if save_label_ids:
        image_save_label_path = os.path.join(image_save_path, "{}-dataset_labelIds".format(dataset_name))
        os.makedirs(image_save_label_path, exist_ok=True)
